...
- Tests: `bunfig.toml`, `src/config.test.ts`, `src/client.test.ts`, `src/generated-contract.test.ts`, `src/openapi-integrity.test.ts`, `src/retained-capabilities.test.ts`, `src/server.test.ts`, `src/stdio-protocol.test.ts`.
...
- The retained v1.5 `tests/` Vitest suite remains historical material; its relevant behavior is covered by the active Bun tests under `src` and is not part of the canonical runner.
